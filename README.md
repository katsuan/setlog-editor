...
- 写真を追加して2秒の静止画クリップ（時刻＋キャプション焼き込み済み）として書き出し、動画から書き出したカットと同じ結合リストに混在させて1本にまとめられる
- 動画書き出しは「①カット書き出し／②結合」の2タブに分離
  - ①カット書き出し: 開いている動画の各ログを個別に2秒の動画（時刻＋キャプション焼き込み済み）として書き出し
  - ②結合: 別々の動画ファイルから書き出したカットもまとめて、並び替えながら1本の動画に結合し、保存/共有（動画を切り替えても書き出し済みカットは保持されます）。「クリップを読み込む」で、以前に保存したクリップ動画ファイルを後から追加することも可能
  - ①タブの各カットには「保存」ボタンがあり、結合前に個別のクリップをファイルとして保存できます
